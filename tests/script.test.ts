...
import Witness from "../lib/bcoin/script/witness";
import Stack from "../lib/bcoin/script/stack";
import Opcode from "../lib/bcoin/script/opcode";
import TX from "../lib/bcoin/primitives/tx";
import consensus from "../lib/bcoin/protocol/consensus";
import { fromFloat } from "../lib/bcoin/utils/fixed";
...
describe("Script", function () {
  it("should recognize a P2SH output", () => {
    ...
    expect(decoded.isScripthash()).toBeTruthy();
  });
  ...
  it("should recognize a Null Data output", () => {
    ...
    const decoded = Script.fromRaw(hex, "hex");
    expect(decoded.isNulldata()).toBeTruthy();
  });
  ...
  it("should handle if statements correctly", () => {
    {
      ...
      expect(stack.items).toEqual([[1], [3], [5]].map((a) => Buffer.from(a)));
    }

    {
      ...
      expect(stack.items).toEqual([[1], [4], [5]].map((a) => Buffer.from(a)));
    }
...

    {
      ...
      expect(stack.items).toEqual([[1], [5]].map((a) => Buffer.from(a)));
    }
  ...
  it("should handle CScriptNums correctly", () => {
    ...
    expect(isSuccess(stack)).toBeTruthy();
  });
    ...
    for (const noCache of [false, true]) {
      ...
      it(`should handle script test ${suffix}:${comments}`, () => {
        ...
        if (expected !== "OK") {
          ...
          expect(err.code).toStrictEqual(expected);
          return;
        }
