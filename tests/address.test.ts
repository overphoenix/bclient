import Network from "../lib/bcoin/protocol/network";
import Address from "../lib/bcoin/primitives/address";
import Script from "../lib/bcoin/script/script";
import nodejsUtil from "node:util";
...
describe("Address", function () {
  it("should match mainnet p2pkh address", () => {
    ...
    expect(addr.toString("main")).toEqual(expectedAddr);
  });
  ...
  it("should match mainnet p2pkh address 2", () => {
    ...
    expect(addr.toString("main")).toStrictEqual(expectedAddr);
  });
  ...
  it("should match testnet p2pkh address", () => {
    ...
    expect(addr.toString("testnet")).toStrictEqual(expectedAddr);
  });

  it("should handle wrong p2pkh hash length", () => {
    const raw = "000ef030107fd26e0b6bf40512bca2ceb1dd80adaa";
    const p2pkh = Buffer.from(raw, "hex");
    expect(() => Address.fromPubkeyhash(p2pkh)).toThrow();
  });
  ...
  it("should match mainnet segwit p2pwsh v0 address", () => {
    ...
    expect(addr.toString("main")).toStrictEqual(
  ...
  it("should match testnet segwit p2pwsh v0 address", () => {
    ...
    const addr = Address.fromWitnessScripthash(p2wpkh);
    expect(addr.toString("testnet")).toStrictEqual(
  ...
  it("should handle invalid segwit hrp", () => {
    ...
    expect(() => Address.fromString(addr, "main")).toThrow();
  });
  ...
  it("should inspect", () => {
    ...
    expect(typeof fmt).toEqual("string");
    expect(fmt.includes("Address")).toBeTruthy();
    expect(fmt.includes("str=")).toBeTruthy();
  });
  ...
  it("should pass all BIP350 test vectors for valid bech32 and bech32m", () => {
    ...
    for (const [addr, script] of vectors) {
      ...
      expect(parsed.isProgram()).toBeTruthy();

      expect(parsed.getType()).toStrictEqual("witness");

      const byte = buffer[0];
      const version = byte ? byte - 0x50 : 0;
      expect(version).toStrictEqual(parsed.version);

      expect(buffer.slice(2)).toEqual(parsed.hash);
    }
  });
  ...
  it("should identify witness (not version 0) address", () => {
    ...
    for (const addr of addresses) {
      ...
      expect(parsed.version).toStrictEqual(1);

      const network = Network.get("regtest");
      expect(parsed.getPrefix(network)).toStrictEqual(parsed.version);
    }
  });
});
