...
common.flags.MANDATORY_VERIFY_FLAGS = common.flags.VERIFY_P2SH;
...
common.flags.STANDARD_VERIFY_FLAGS =
  ...
  common.flags.VERIFY_DERSIG |
  common.flags.VERIFY_STRICTENC |
  common.flags.VERIFY_MINIMALDATA |
  common.flags.VERIFY_NULLDUMMY |
  common.flags.VERIFY_DISCOURAGE_UPGRADABLE_NOPS |
  common.flags.VERIFY_CLEANSTACK |
  common.flags.VERIFY_MINIMALIF |
  common.flags.VERIFY_NULLFAIL |
  common.flags.VERIFY_CHECKLOCKTIMEVERIFY |
  common.flags.VERIFY_CHECKSEQUENCEVERIFY |
  common.flags.VERIFY_LOW_S |
  common.flags.VERIFY_WITNESS |
  common.flags.VERIFY_DISCOURAGE_UPGRADABLE_WITNESS_PROGRAM |
  common.flags.VERIFY_WITNESS_PUBKEYTYPE |
  common.flags.VERIFY_CONST_SCRIPTCODE;
...
common.flags.ONLY_STANDARD_VERIFY_FLAGS =
  common.flags.STANDARD_VERIFY_FLAGS & ~common.flags.MANDATORY_VERIFY_FLAGS;
...
common.isHashType = function isHashType(sig) {
  assert(Buffer.isBuffer(sig));

  if (sig.length === 0) return false;

  const type = sig[sig.length - 1] & ~common.hashType.ANYONECANPAY;

  if (type < common.hashType.ALL || type > common.hashType.SINGLE) return false;
...
common.isLowDER = function isLowDER(sig) {
  if (!common.isSignatureEncoding(sig)) return false;
...
common.isKeyEncoding = function isKeyEncoding(key) {
...
common.isCompressedEncoding = function isCompressedEncoding(key) {
  assert(Buffer.isBuffer(key));
...
common.isSignatureEncoding = function isSignatureEncoding(sig) {
  assert(Buffer.isBuffer(sig));
...
common.toASM = function toASM(item, decode) {
  if (item.length <= 4) {
    const num = ScriptNum.decode(item);
    return num.toString(10);
  }

  if (decode && common.isSignatureEncoding(item)) {
    const type = item[item.length - 1];

    let symbol = common.hashTypeByVal[type & 0x1f] || "";

    if (symbol) {
      if (type & common.hashType.ANYONECANPAY) symbol += "|ANYONECANPAY";
      symbol = `[${symbol}]`;
    }
