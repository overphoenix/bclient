...
policy.MAX_TX_SIZE = consensus.MAX_BLOCK_SIZE / 10;
...
policy.MAX_TX_WEIGHT = consensus.MAX_BLOCK_WEIGHT / 10;
...
policy.MAX_TX_SIGOPS = consensus.MAX_BLOCK_SIGOPS / 5;
...
policy.MAX_TX_SIGOPS_COST = consensus.MAX_BLOCK_SIGOPS_COST / 5;
...
policy.FREE_THRESHOLD = (consensus.COIN * 144) / 250;
...
policy.MEMPOOL_MAX_ANCESTORS = 25;
...
policy.MEMPOOL_MAX_SIZE = 100 * 1000000;
...
policy.MEMPOOL_EXPIRY_TIME = 72 * 60 * 60;
...
policy.MEMPOOL_MAX_ORPHANS = 100;
...
policy.MAX_BLOCK_WEIGHT = 1000000 * consensus.WITNESS_SCALE_FACTOR;
...
policy.BLOCK_PRIORITY_WEIGHT = 0;
...
policy.BLOCK_PRIORITY_THRESHOLD = policy.FREE_THRESHOLD;
...
policy.getMinFee = function getMinFee(size, rate) {
  if (rate == null) rate = policy.MIN_RELAY;
...
policy.getRoundFee = function getRoundFee(size, rate) {
  if (rate == null) rate = policy.MIN_RELAY;
...
policy.getRate = function getRate(size, fee) {
  assert(size >= 0);
  assert(fee >= 0);
